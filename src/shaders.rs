...
pub fn vertex_shader(vertex: &Vertex, uniforms: &Uniforms) -> Vertex {
    ...
    let transformed = uniforms.projection_matrix * uniforms.view_matrix * uniforms.MM * position;
    ...
    let model_mat3 = mat4_to_mat3(&uniforms.MM);
...
pub fn planeta_gaseoso(fragment: &Fragment, uniforms: &Uniforms) -> Color {
    ...
    let patron1 =
        ((distancia + ruido) * 7.0 * frecuencia + (y + ruido) * 5.0 + angulo).sin() * 0.5 + 0.5;
    let patron2 = ((distancia + ruido) * 5.0 * frecuencia - (y + ruido) * 8.0 + PI / 3.0 + angulo)
        .sin()
        * 0.5
        + 0.5;
    let patron3 =
        ((distancia + ruido) * 6.0 * frecuencia + (x + ruido) * 4.0 + 2.0 * PI / 3.0 + angulo)
            .sin()
            * 0.5
            + 0.5;
...
pub fn black_and_azul(fragment: &Fragment, uniforms: &Uniforms) -> Color {
    ...
    let deciNaZU = if random_number < 50 {
        Color::new(0, 0, 0)
    } else {
        Color::new(255, 255, 255)
    };

    deciNaZU * fragment.intensity
}
...
pub fn shader_agujero_negro(fragment: &Fragment, uniforms: &Uniforms) -> Color {
    ...
    let dist =
        ((distance_from_center - radio_interno) / (radio_externo - radio_interno)).clamp(0.0, 1.0);
